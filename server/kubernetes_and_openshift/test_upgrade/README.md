...
1. **Create test-upgrade configmap**
   ...
   ```SHELL
   $ kubectl apply -f https://raw.githubusercontent.com/khulnasoft/deployments/2022.4/server/kubernetes_and_openshift/test_upgrade/001_khulnasoft_test_upgrade_configMap.yaml
   ```
...
2. **Create test-upgrade secrets**
   ...
   ```shell
   $ kubectl apply -f https://raw.githubusercontent.com/khulnasoft/deployments/2022.4/server/kubernetes_and_openshift/test_upgrade/002_khulnasoft_test_uprade_secrets.yaml
   ```
...
3. **Deploy the Console testing the ugprade**
  ...
  Deployment of the Console should result in a successful or error output. This indicates the database's ability to upgrade to the new version.
   ...
   ```shell
   $ kubectl apply -f https://raw.githubusercontent.com/khulnasoft/deployments/2022.4/server/kubernetes_and_openshift/test_upgrade/003_khulnasoft_test_upgrade_job.yaml
   ```
