...
pipeline {
    ...
    stages {
        ...
        stage('Cloudformation Tunnel Scan') {
            when {
                allOf {
                ...
                script {
                    dir("deployments") {
                        parallel changedCfFiles.collectEntries { filename ->
                            ...
                            shortName = "${shortName}".replaceAll(/khulnasoft|\.yaml/, '')

                            ["${shortName}": {
                                stage("Tunnel scan ${shortName}") {
                                    docker.withRegistry('https://khulnasoftdev.azurecr.io', 'khulnasoftdev-push') {
                                        docker.image("khulnasoftdev.azurecr.io/helm-cicd").inside("-u root") {
                                            log.info "Starting Tunnel scan for file: ${filename}"
                                            sh "tunnel config --severity HIGH,CRITICAL --ignorefile .tunnelignore --exit-code 1 ${filename}"
                                        }
                                    }
                                }
                            }]
            ...
            steps {
                script {
                    docker.image("${pythonImage}").inside("-u root") {
                        ...
                        sh "aws --region ${env.AWS_REGION} codeartifact login --tool pip --repository deployment --domain khulnasoft-deployment --domain-owner ${env.AWS_ACCOUNT_ID}"
                        sh "pip install --no-build-isolation khulnasoft-deployment"
                        ...
                        parallel changedCfFiles.collectEntries { filename ->
                            ...
                            ["${shortName}": {
                                stage("Verify and Deploy ${shortName}") {
                                    ...
                                    if (filename.contains("khulnasoft-ecs-fargate")) {
                                        clusterName = "far-${baseName}"
                                        testFile = "tests/fargate/test_cloudformation.py"
                                    } else if (filename.contains("khulnasoft-ecs-ec2")) {
