...
# for using custom namespace instead of KHULNASOFT NS download the 001_kube_enforcer_config.yaml, make changes to it and keep it in current directory where this script is running
_prepare_ke() {          
    ...
    else                                # for deploying kube enforcer in default namespace, i.e., KHULNASOFT.
        printf "\nInfo: Local config file not found, attempting to download from GitHub\n"
        githubBranch="2022.4"
        if curl https://raw.githubusercontent.com/khulnasoft/deployments/$githubBranch/enforcers/kube_enforcer/kubernetes_and_openshift/manifests/kube_enforcer/001_kube_enforcer_config.yaml -o "$local_config_file"; then
