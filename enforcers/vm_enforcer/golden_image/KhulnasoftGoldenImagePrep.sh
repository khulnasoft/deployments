...
display_usage()
{
	echo "Khulnasoft Security Golden Image Preparation script"
...
{
	echo
	echo -n "The Khulnasoft Golden Image preparation script will perform irreversible cleanup actions on the VM Enforcer. Proceed? [Y/N]: "
	read user_input
	[ "$user_input" != "y" ] && exit 0
}
...
detect_install_dir()
{
	...
	if [ -f /etc/khulnasoft ]; then
		# Try reading installation directory path from the configuration file.
		dbg "[debug]: trying to read installation path from file /etc/khulnasoft"
		KHULNASOFT_ROOT_DIR=$(cat /etc/khulnasoft)
	fi
	if [ ! -d "$KHULNASOFT_ROOT_DIR" ]; then
		dbg "[debug]: trying /opt/khulnasoft as installation path"
		KHULNASOFT_ROOT_DIR=/opt/khulnasoft 
	fi
	if [ ! -d "$KHULNASOFT_ROOT_DIR" ]; then
		dbg "[debug]: trying /var/lib/khulnasoft as installation path"
		KHULNASOFT_ROOT_DIR=/var/lib/khulnasoft
	fi
	if [ ! -d "$KHULNASOFT_ROOT_DIR" ]; then
		# Try reading installation directory path from the environment variable.
		dbg "[debug]: trying to read installation path from KHULNASOFT_INSTALL_PATH environment variable"
		KHULNASOFT_ROOT_DIR=$(printenv KHULNASOFT_INSTALL_DIR)
	fi
	if [ ! -d "$KHULNASOFT_ROOT_DIR" ]; then
...
stop_vm_enforcer()
{
	...
	vm_enforcer_service="khulnasoft-enforcer"
	rc=0
	for i in {1..3}; do
...
delete_vm_enforcer_data()
{
	...
	db_dir="$KHULNASOFT_ROOT_DIR"/data	
	rm -rf "$db_dir"/*
	rm -rf "$db_dir"/guid
}
...
create_golden_image_file()
{
	...
	touch "$KHULNASOFT_ROOT_DIR"/GOLDEN_IMAGE
}
